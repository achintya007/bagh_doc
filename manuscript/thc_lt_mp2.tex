...
$\Ngrid\times\Ngrid$ or $\Ngrid\times n_{\mathrm{mo}}^2$ array. The direct
...
When the reference is run with the THC option the X2CAMF driver supplies
$X^{o}$, $X^{v}$ and $Z$ directly. Otherwise they are built here from the
density-fitted integrals without forming any array of grid-squared or
grid$\times$orbital-squared size:
\begin{enumerate}
\item The two-component collocation
  $X_{kPp}=w_P^{1/4}\sum_{\mu}\phi^{(k)}_{\mu}(\rr_P)\,C_{\mu p}$
  is evaluated in blocks of grid points, $w_P$ the quadrature weight.
\item Interpolation points are chosen by a pivoted Cholesky factorisation of
  ...
  $S_{PQ}=\sum_{pq}\rho_{P}(pq)^{*}\rho_{Q}(pq)
        =\sum_{kk'}\bigl|\sum_p X_{kPp}X^{*}_{k'Qp}\bigr|^{2}$,
  with the Schur-updated diagonal maintained explicitly and each required
  column of $S$ recomputed on demand in $\Ord{\Ngrid n_{\mathrm{mo}}}$ work;
  no $\Ngrid\times\Ngrid$ matrix is ever stored. Selection stops when the
  updated diagonal falls below a relative floor, so $K$ is rank-adaptive.
\item The collocation is gathered on the $K$ retained points to give
  $X^{o}$ and $X^{v}$.
\item The THC core solves the least-squares problem
  $Z = S^{-1} E\, S^{-1}$ with
  $E_{PQ}=\sum_{L}\bigl(\sum_{pq}\rho_P(pq)L^{L}_{pq}\bigr)
                  \bigl(\sum_{rs}\rho_Q(rs)L^{L}_{rs}\bigr)$,
  the pair densities on the retained points formed in blocks of $P$ and the
  auxiliary index streamed; $E$ is accumulated as one $K\times\Naux$ panel
  times its transpose. $S$ is regularised with a small multiple of its
  trace and inverted by a Cholesky solve (a pseudoinverse with an explicit
  cutoff as fallback). $Z$ is symmetrised.
\end{enumerate}
...
guarded so that a request whose $K\,n_{\mathrm{mo}}^{2}$ working set would
not fit is refused with a diagnostic rather than thrashing.
...
spinor counts; $\Naux$ auxiliary functions; $K$ retained interpolation
points ($K\sim c\,n_{\mathrm{mo}}$, $c\sim 8$--$15$); $\Ngrid$ parent grid
points; $\Ngrid^{\mathrm{LT}}$ Laplace nodes; $p$ the $P$-block size in the
ISDF build.}
\label{tab:cost}
\begin{ruledtabular}
\begin{tabular}{lll}
...
\colrule
ISDF points \& core & $K\,\Naux\,n_{\mathrm{mo}}^{2}$ &
   $K^{2}+p\,n_{\mathrm{mo}}^{2}$ \\
...
$\Naux n_{\mathrm{mo}}^2$ in memory. The direct diagram is the
rate-determining arithmetic at $\Ord{K^3}$ per node; with the geometric
Laplace quadrature $\Ngrid^{\mathrm{LT}}\sim 10$--$20$ suffices for
$10^{-6}$--$10^{-7}$ relative accuracy on the denominator. The exchange
...
avoids every grid-squared array, the direct kernel has an out-of-core path
that streams the THC core, and the exchange kernel takes a Schwarz pair
screen. With the exchange diagram removed the method is a scaled-opposite-
spin relativistic MP2 that scales to the largest spinor bases; the
remaining obstacle to a full relativistic MP2 at that size is a
pair-domain truncation of the exchange, which is left for future work.
