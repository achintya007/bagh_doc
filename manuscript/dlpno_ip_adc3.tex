...
The routing question here is different from that of
Section~\ref{sec:routing}, because the target indices $a,b$ are being
\emph{constructed} rather than met: there is no third pair that owns the
summed index $c$. The first two terms are safe --- the particle ladder never
leaves the pair, and the $oooo$ term sums no virtual against an integral --- and
are evaluated in the pair's PNO space. In the remaining eight, $c$ belongs to
the summed occupied label $k$ and the amplitude comes from a neighbouring
pair. These are assembled in the pair's non-redundant semicanonical domain
basis $\mathbf{d}^{ij}_{\text{nr}}$ of Eq.~(\ref{eq:dpno}), which spans the
whole domain and therefore truncates nothing the neighbour brings, and
projected into the PNO space only once, at the end:
%
\begin{equation}
  \mathcal{D}^{ij} \;\mathrel{+}=\;
    (\mathbf{v}^{ij})^{\dagger}\,\mathcal{D}^{ij}_{\text{nr}}\,\mathbf{v}^{ij}.
  \label{eq:proj_nr}
\end{equation}
...
\paragraph{Step 1: multipole prescreening.}
The exchange integral between two well-separated charge distributions is
dominated by their dipole--dipole interaction, giving the estimate
%
\begin{equation}
...
Equation~(\ref{eq:dipole}) is meaningful only between distributions that do
not overlap. Pairs whose occupied--occupied differential overlap exceeds
$\TCut{DO}_{ij}$ are therefore retained unconditionally and no estimate is
formed for them. The purpose of step 1 is not to make the strong/weak decision
but to keep step 2 affordable; it is applied at $\TCut{Pre}$, two orders of
magnitude looser than $\TCut{Pairs}$.
...
\begin{equation}
  E_{\text{weak}} = \sum_{ij\,\in\,\text{weak}} e_{ij}
    + \sum_{ij\,\in\,\text{step 1}} e^{\text{dip}}_{ij},
  \label{eq:eweak}
\end{equation}
%
using the best estimate available for each: the semicanonical pair energy for
pairs step 2 rejected, and the dipole estimate for those step 1 rejected
without ever computing one. $E_{\text{weak}}$ is a real approximation to the
...
The method is implemented in the BAGH program package. Orbital localization,
